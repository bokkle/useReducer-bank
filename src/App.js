...
as an analogy to explain how useReducer works, but it's simplified 
...
deposit amount of 500 to open an account (which means that the balance 
will start at 500)

5. Customer can only request a loan if there is no loan yet. If that condition is met, 
the requested amount will be registered in the 'loan' state, and it will be 
...
but that's no problem, because the customer can't close their account now 
(see next point)
  ...
  switch (action.type) {
    ...
    case "payLoan":
      return {
        ...state,
        balance:
          state.loan > 0 && state.balance > 5000
            ? state.balance - action.payload
            : state.balance,
        loan: state.balance > 5000 ? initialState.loan : state.loan,
      };
    case "closeAccount":
    ...
    <div className="App">
      ...
      <p>
        <button
          ...
          disabled={balance >= 5000 && loan > 0 ? false : true}
        >
          Pay loan
        </button>
      </p>
      <p>
        <button
          ...
          disabled={isActive && balance === 0 && loan === 0 ? false : true}
        >
          Close account
        </button>
      </p>
